...
fn main() {
    ...
    l.push_back(Node{i:r-1, j:c-1, l:0, r:0});
    ...
    while l.len() > 0 || sl.len() > 0 {
        ...
        if front.i > 0 && !visited[front.i-1][front.j] && s[front.i-1][front.j] != '*' {
			...
			l.push_back(Node{i:front.i - 1, j:front.j, l:front.l, r:front.r});
		}
		...
		if front.j > 0 && !visited[front.i][front.j-1] && s[front.i][front.j-1] != '*' && front.l < x {
			...
			sl.push_back(Node{i:front.i, j:front.j - 1, l:front.l + 1, r:front.r});
		}
		if front.j+1 < m && !visited[front.i][front.j+1] && s[front.i][front.j+1] != '*' && front.r < y {
			ans += 1;
			visited[front.i][front.j+1] = true;
			sl.push_back(Node{i:front.i, j:front.j + 1, l:front.l, r:front.r + 1});
		}
    }
    println!("{}", ans);
}
